...
export default function SearchForm({ onAddMovies }: SearchFormProps) {
  ...
  const { results, selectedMovieIds, setSelectedMovieIds } =
  ...
  useEffect(() => {
    setSelectedMovieIds((prevIds) => {
      const searchResultIds = results.map((movie) => movie.id);
      const result = prevIds.filter((id) => searchResultIds.includes(id));

      return [...new Set([...prevIds, ...result])];
    });
  }, [results]);
      ...
      >
        ...
        >
          <Button
            ...
            onClick={() =>
              onAddMovies(
                results.filter((movie) => selectedMovieIds.includes(movie.id))
              )
            }
          >
